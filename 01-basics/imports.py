...
today = datetime.now()
today2 = date.today()
year = rrule(freq=YEARLY, count=5, dtstart=today2, bymonth=8, bymonthday=13, byweekday=SU)[0].year
year2 = rrule(freq=YEARLY, dtstart=today2, bymonth=12, bymonthday=24, byweekday=SU)[0].year

rdelta = relativedelta(easter(year), today2)

print('Dnes je:', today)
todayAleNapsaneNormalne = today.strftime("%d/%m/%Y %H:%M:%S")
print("Dnes je:", todayAleNapsaneNormalne)

print("Dalsi velikonocni nedele jsou:")
for i in range(0,5):
    rdelta = relativedelta(easter(year), today2)
    year += 1
    print('%s' % (today2+rdelta))

print('Stedry den v nedeli bude roku: ', year2)
